function getFindText() {
  ...
  for (let i = 0; i < getContentsValue.length; i++) {
    if (SearchValue === getContentsValue[i]) getContentsValue[i] = `<p class="findedText_${i} textEmphasize">${SearchValue}</p>`;
  }
  ...
  for (let i = 0; i < changedSelectedValue; i++) {
    ...
    optionElement.textContent = SearchValue;
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  document.querySelector(".selectList").addEventListener("change", (e) => {
    ...
    console.log(findedList[1].textContent);
